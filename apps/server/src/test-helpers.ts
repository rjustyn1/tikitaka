...
export interface FakeRunnerOptions {
  ...
  outputFor?: (request: GroupRunnerRequest) => string;
  /** Return a message to make that run fail; return `null` to let it succeed. */
  failFor?: (request: GroupRunnerRequest) => string | null;
  /** Defaults to the incoming thread id, or `"fake-thread"` on a new thread. */
  threadIdFor?: (request: GroupRunnerRequest) => string | null;
}
...
export class FakeRunner implements AgentRunner {
  readonly requests: GroupRunnerRequest[] = [];
  ...
  requestsFor(agentId: string): GroupRunnerRequest[] {
  ...
  async run(request: RunnerRequest): Promise<RunnerResult> {
    const groupRequest = request as GroupRunnerRequest;
