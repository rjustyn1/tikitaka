...
import {
  deriveMemberAgentIds,
  findMembershipError,
  readMembers,
  type AgentLease,
  type AgentLeaseHolder,
  type CreateGovernedGroupInput,
  type GovernedGroup,
  type GroupMember,
  type GroupRunnerRequest,
  type UpdateGovernedGroupInput,
} from "./pending-contracts.js";
...
export class GroupRunner {
  ...
  async createGroup(input: CreateGovernedGroupInput): Promise<AgentGroup> {
    ...
    const timestamp = now();
    const group: GovernedGroup = {
      id: randomUUID(),
      name: input.name.trim(),
      description: input.description?.trim() ?? "",
      members: input.members,
      // Derived mirror of `members`, kept until Person 1 lands A4.
      memberAgentIds: deriveMemberAgentIds(input.members),
      activeTaskId: null,
      createdAt: timestamp,
      updatedAt: timestamp,
    };
  ...
  async updateGroup(
    ...
    return this.store.mutate((database) => {
      ...
      if (input.members) {
        this.assertAgentsExist(database, input.members);
        (group as GovernedGroup).members = input.members;
        group.memberAgentIds = deriveMemberAgentIds(input.members);
        this.syncParticipants(database, group as GovernedGroup, timestamp);
      }
  ...
  private async runPlanNode(
    ...
    try {
      ...
      const request: GroupRunnerRequest = {
        agentId: node.agentId,
        runId,
